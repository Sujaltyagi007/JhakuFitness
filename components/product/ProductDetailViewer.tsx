...
const ProductViewer3D = dynamic(() => import("@/components/three/ProductViewer3D"), {
  ssr: false,
  loading: () => (
    <div className="flex h-full w-full animate-pulse items-center justify-center bg-linear-to-br from-ink to-ink-soft">
      <p className="text-sm text-white/50">Loading 3D viewer…</p>
    </div>
  ),}
);

export default function ProductDetailViewer({ kind, colorway }: { kind: ModelKind; colorway: Colorway; }) {
