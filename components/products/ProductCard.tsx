  ...
  return (
    <Link href={`/products/${product.slug}`} className="group block">
      <motion.div ref={ref}
        ...
        className="overflow-hidden rounded-2xl border border-theme-border bg-theme-card backdrop-blur-md shadow-lg transition-all duration-300 group-hover:border-gold/50 group-hover:shadow-2xl group-hover:shadow-gold/10"      >
        <div className="relative flex aspect-4/3 items-center justify-center overflow-hidden"
          style={{
            background: `radial-gradient(120% 120% at 30% 20%, ${accentColor}33, transparent 60%), linear-gradient(160deg, ${bodyColor}, #121215)`,
          }}>
          {product.imageUrl ? (
            ...
            <ModelIcon kind={product.modelKind} className="h-24 w-24 text-white/90 transition-transform duration-500 group-hover:scale-110" />
          )}
          {product.featured && (
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-black line-clamp-2">{product.tagline}</p>
            </div>
